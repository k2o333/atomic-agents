#!/usr/bin/env python3
"""
Test script for the complete Notify Handler component.

This script tests the complete notify handler functionality by:
1. Setting up the necessary connections
2. Running the notify handler in a separate process
3. Simulating task creation/update in the database
4. Verifying that NOTIFY events are processed by the handler
...
def test_complete_notify_handler():
    """Test the complete notify handler functionality."""
    ...
    # Start the notify handler as a subprocess
    print("Starting notify handler process...")
    handler_process = subprocess.Popen([
        '/root/projects/atom_agents/myenv311/bin/python',
        'PersistenceService/db_triggers/start_notify_handler.py',
        '--redis-host', 'localhost',
        '--redis-port', '6379',
        '--task-queue', test_queue
    ], cwd='/root/projects/atom_agents')
    
    # Give the handler a moment to start
    time.sleep(3)
    
    # Check if the process is running
    if handler_process.poll() is not None:
        print("✗ Notify handler failed to start")
        print(handler_process.stdout)
        print(handler_process.stderr)
        return
    
    print("Notify handler started successfully.")
    ...
    # Terminate the notify handler process
